    ...
    def tolk_kart(self) -> None:
        ...
        for y in range(len(rå_data)):
            for x in range(len(rå_data[y])):
                ...
                if type(tile) == str:
                    _ = Tile(x,y,størrelse,self.tolket_kart, sprite="Sprites/gulv1.png", patruljepunkt=int(tile[1:]))
                    if (tile[0]) in self.patruljePunkter: self.patruljePunkter[(tile[0])].append(_)
                    else: self.patruljePunkter[(tile[0])]= [_]
